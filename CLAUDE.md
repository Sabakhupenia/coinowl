...
1. **v0.7.3 — Alerts & subscriptions:** unified background-watcher feature with **asymmetric delivery**: price alerts push real-time on threshold cross, scheduled summaries defer to the user's next message so the bot is never intrusive. Both go through a one-shot **personality-wrap LLM call** that reads back the user's `original_phrasing` column to make the push sound like a continuation of the conversation ("hey, as you asked — BTC just hit 80k"). Tables: `alerts(symbol, threshold, direction, recurring, original_phrasing, ...)` — `recurring=false` default, auto-disables on first cross; `scheduled_pushes(cron_expr, tool_name, tool_args_json, original_phrasing, ...)`; `pending_notifications(schedule_id FK, payload_json, delivered_at)` — **UPSERT on schedule_id** so a user who's been offline for days only sees the most recent summary, not stale duplicates. New LLM tools: `set_price_alert`, `list_price_alerts`, `cancel_price_alert`, `schedule_push`, `list_scheduled_pushes`, `cancel_scheduled_push`. Background watcher is one `asyncio.create_task` started in `bot/main.py:run()`, wakes ~every 30s. Multiple deferred items get **batched into a single coherent prefix message** by the personality LLM, not delivered as a string of beeps.
